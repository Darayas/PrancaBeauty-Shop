...
namespace PrancaBeauty.Application.Apps.Countries
{
    public class CountryApplication : ICountryApplication
    {
        ...
        public CountryApplication(ICountryRepository countryRepository, ILogger logger, ILocalizer localizer, IServiceProvider serviceProvider)
        {
        ...
        public async Task<List<OutGetCourtriesListForCombo>> GetListForComboAsync(InpGetListForCombo Input)
        {
            try
            {
                #region Validation
                ...
                var qData = await _CountryRepository.Get
                                                   .Where(a => a.IsActive)
                                                   .Select(a => new OutGetCourtriesListForCombo
                                                   {
                                                       Id = a.Id.ToString(),
                                                       Name = a.Name,
